...
struct json_tag_base {
  ...
  constexpr json_tag_base(CharT const* data, size_type size) noexcept : data_{ data }, size_{ size } {}
  constexpr operator bool() const noexcept { return (data_ != nullptr) && (size_ > 0); }
...
static_assert(std::is_default_constructible_v<json_tag_base<>>, "not default constructible");
...
public:
  ...
  constexpr json_integer_tag(CharT const* number_stt_ptr, size_type size, bool is_neg) noexcept
  ...
  constexpr static std::pair<self_type, CharT const*> tag(CharT const* b, CharT const* e) noexcept {
    if (b >= e) {
      return { {}, e };
    }
...
template <typename CharT>
std::basic_ostream<CharT>& operator<<(std::basic_ostream<CharT>& os, json_integer_tag<CharT> const& self) {
  if (self.is_neg_) {
    os << '-';
  }
...
public:
  ...
  constexpr static std::pair<self_type, CharT const*> tag(CharT const* _b, CharT const* e, std::size_t fb) noexcept {
    auto b = _b + fb;
  ...
  parse_result<V> parse() const noexcept {
    V parsed;
    auto from_chars_ret = std::from_chars(this->data_, this->data_ + this->size_, parsed);
...
template <typename CharT>
std::basic_ostream<CharT>& operator<<(std::basic_ostream<CharT>& os, json_float_tag<CharT> const& self) {
  return os.write(self.data_, self.size_);
}
...
template<typename T>
concept lazyjson_type_protocol = std::derived_from<T, json_primitive<T>> &&
...
template <typename T>
concept lazyjson_lazy_type_protocol = lazyjson_type_protocol<T> && requires (T a) {
...
public:
  ...
  constexpr json_integer(std::in_place_type_t<U> inplace_holder, Args &&... args) noexcept
    : item{ inplace_holder, std::forward<Args>(args)... }
  {
  }
  constexpr json_integer(value_type val) noexcept : item{ std::in_place_type<value_type>, val } {}
  ...
  friend std::basic_ostream<CharU>& operator<<(std::basic_ostream<CharU> &os, json_integer<IntU, CharU> const &obj);
};

static_assert(lazyjson_lazy_type_protocol<json_integer<int>>);

template<std::integral IntT, typename CharT>
std::basic_ostream<CharT>& operator<<(std::basic_ostream<CharT> &os, json_integer<IntT, CharT>const &obj) {
  std::visit([&os](auto&& parsed) -> void {
    os << parsed;
  }, obj.item);
  return os;
}
...
public:
  template<typename U, typename ...Args>
  constexpr json_float(std::in_place_type_t<U> inplace_holder, Args &&... args) noexcept
    : super_type{}, item{ inplace_holder, std::forward<Args>(args)... }
  { }
  constexpr json_float(value_type val) noexcept : item{ std::in_place_type<value_type>, val } {}
  ...
  friend std::basic_ostream<CharU>& operator<<(std::basic_ostream<CharU> &os, json_float<FloatU, CharU> const &obj);
};
...
template<std::floating_point FloatU, typename CharU>
std::basic_ostream<CharU>& operator<<(std::basic_ostream<CharU> &os, json_float<FloatU, CharU>const &obj) {
  std::visit([&os](auto&& parsed) -> void {
    os << parsed;
  }, obj.item);
  return os;
...
protected:
  static constexpr std::tuple<std::string, std::size_t, std::errc> unescape_if(std::string_view s) {
  ...
  static constexpr std::tuple<std::string, std::size_t, std::errc> unescape(std::string_view s) {
...
public:
  ...
  value_type get() const noexcept {
    return std::visit([](const auto &visited) noexcept -> value_type {
      return visited;
    }, this->item);
  }
  ...
  std::size_t size() const noexcept {
    return std::visit([](auto const& visited) noexcept -> std::size_t {
      return visited.size();
    }, this->item);
  }
...
inline std::ostream& operator<<(std::ostream &os, json_key const &obj) {
  os << '"';
  std::visit([&os]<class T>(T const& parsed) {
...
inline std::strong_ordering operator<=>(json_key const& a, json_key const& b) noexcept  {
  return a.get() <=> b.get();
}
...
inline std::strong_ordering operator<=>(json_key const& a, std::string_view b) noexcept {
  return a.get() <=> b;
}
...
public:
  ...
  using element_type = std::variant<
    tag_type, // tag without escape seq
    tag_type, // tag with escape seq (converted to value type when get called)
    parsed_type>; 
...
public:
  ...
  constexpr explicit json_string(std::in_place_index_t<I> ipi, Args &&... args) noexcept(I <= json_string::esc_inc_tag_idx)
  ...
  std::errc convert() const {
    if(this->item.index() == json_string::esc_inc_tag_idx) {
      auto &&[unescaped, _, er] = this->unescape(
...
inline std::ostream& operator<<(std::ostream &os, json_string const &obj) {
  std::visit([&os]<class T>(T const& v) {
...
public:
  ...
  constexpr static std::pair<std::size_t, self_type> tag(value_type s, json_integer_tag<CharT> const& deci_tag) noexcept {
    std::size_t i{};
    ...
    if (new_size >= super_type::npos || fstt >= super_type::npos || fsize >= super_type::npos || estt >= super_type::npos) {
      return { value_type::npos, {} };
    }
    return { i, self_type{
      deci_tag.data_,
      ...
      deci_tag.is_neg_,
      deci_size,
      static_cast<uint16_t>(fstt),
      static_cast<uint16_t>(fsize),
      static_cast<uint16_t>(estt),
      e_is_neg } };
  }
  ...
  constexpr V parse() const noexcept {
    ...
    auto deci = json_integer_tag<CharT>{
      this->data_, this->deci_size_, this->is_neg_ }.template parse<int64_t, false>();

    uint64_t fnum;
    uint64_t fden;
    if (f_exists) { // point exists
      fnum = json_integer_tag<CharT>{ this->data_ + this->fp_stt_, this->fp_size_, false}
    ...
    if (e_exists) {
      ...
      sci_exp = json_integer_tag<CharT>{this->data_ + this->e_deci_stt_, e_deci_size, this->e_is_neg_}
